...
- Prefer concrete technologies/libraries/frameworks over generic role labels.
- Use the EXACT schema provided below.
- Return a JSON object with EXACTLY the keys in the schema. Do not include any other keys.

SCHEMA:
${JSON.stringify(EMPTY_EXTRACTION_SCHEMA, null, 2)}

DEFINITIONS:
- "extractedSkills": Array of { "skill": string } for skills explicitly mentioned in the resume text.
- "inferredSkills": Array of { "skill": string, "source": string } for skills that can be reasonably inferred from context. 
   Example: If the resume mentions “worked on REST APIs”, infer "API Integration".
   Each inferred skill MUST have a source.
- "experienceYears": Numerical estimate based on text (use null if unclear).
- "educationLevel": Highest education level mentioned.
- "tools": Developer tools explicitly mentioned (array of strings).
- "projects": Brief project titles extracted from resume text (array of strings).
